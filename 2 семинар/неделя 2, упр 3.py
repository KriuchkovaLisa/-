f = list(input().split())
res = ''.join(f)
l = res[::-1]#оборотка к строке
a = res.replace('E', '*').replace('J', '?').replace('S', '!').replace('Z', '-')
a = a.replace('3', '@').replace('L', '#').replace('2', '$').replace('5', '+')
a = a.replace('*', '3').replace('?', 'L').replace('!', '2').replace('-', '5')
a = a.replace('@', 'E').replace('#', 'J').replace('$', 'S').replace('+', 'Z')#зеркальное отображение
zer = a[::-1]#оборотка к зеркальному отображению
